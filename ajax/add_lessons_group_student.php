...
// lessons_group_id - ID группы уроков, student_grade - оценка студента (1 или 0), student_id - ID студента
if (!isset($_GET['lessons_group_id']) || !isset($_GET['student_grade']) || !isset($_GET['student_id'])) {
...
// Формируем SQL-запрос для вставки данных в таблицу lessons_group_student
// lessons_group_student_id - автоинкрементное поле, поэтому указываем NULL
// lessons_group_id - ID группы уроков из параметра
...
$sql = sprintf("INSERT INTO `lessons_group_student` (`lessons_group_student_id`, `lessons_group_id`, `student_id`, `student_grade`) VALUE (NULL, '%s', '%s', '%s')",
                $_GET['lessons_group_id'],      // ID группы уроков
