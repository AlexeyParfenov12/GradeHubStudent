...
<h2 class="addGroup">Добавить задание</h2>
<form action="<?= BASE_URL ?>controllers/add_lesson.php" method="post">
  <div class="addGroupeInputButton">
    <input class="addGroupeInput" type="text" name="lesson_name" />
    <input type="submit" class="addGroupButton" value="Добавить">
  </div>
</form>
<ol class="studentList">
  <? echo fnLessonItems($_SESSION['course_id']); ?>
</ol>

<div class="modal" id="modalEdit">
  <p style="color:#0056b3; text-align: center;">Изменить имя задания</p>
...
<script>
  ...
  function confirmEdit() {
    if (currentForm) {
      ...
      let hidden = currentForm.querySelector('input[name="lesson_name"]');
      if (!hidden) {
        hidden = document.createElement('input');
        hidden.type = 'hidden';
        hidden.name = 'lesson_name';
