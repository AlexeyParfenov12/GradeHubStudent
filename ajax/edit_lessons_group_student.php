...
// lessons_group_student_id - ID записи в таблице lessons_group_student
// student_grade - новая оценка студента (1 или 0)
if (!isset($_GET['lessons_group_student_id']) || !isset($_GET['student_grade'])) {
...
// Формируем SQL-запрос для обновления данных в таблице lessons_group_student
// Обновляем поле student_grade для записи с указанным lessons_group_student_id
$sql = sprintf("UPDATE `lessons_group_student` SET `student_grade`='%s' WHERE `lessons_group_student_id` = '%s'",
    ...
    $_GET['lessons_group_student_id']   // ID записи в таблице lessons_group_student
);
